...
import {
  TourCharacteristic,
  TransportType,
} from '../../tour-authoring/tour/model/tourCharacteristic.model';
  ...
  template: `
    <div class="example-container">
      ...
      <form [formGroup]="compositeTourForm">
        ...
        <input
          matInput
          [formControl]="compositeTourForm.controls['name']"
          type="text"
        />
      </form>
      <button class="button" (click)="connectTours()">
        Connect these tours
      </button>
    </div>
  `,
...
export class ConfirmationDialog {
  ...
  calculateDifficultyLevel(): DifficultyLevel {
    ...
    var num = 0;
    this.todo.forEach((tour) => {
      if (tour.difficultyLevel == DifficultyLevel.Easy) difficulty += 1;
    ...
    else if (level == 2) return DifficultyLevel.Medium;
    else return DifficultyLevel.Hard;
  }

  findComplexTourEquipment(): Observable<number[]> {
    const observables = this.todo.map((tour) =>
      this.toureqService.getEquipmentForTour(tour.id || 0)
    );
